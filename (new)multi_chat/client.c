...
/*compile this file : $gcc server.c -std=c99 -lpthread -Wall -o server
  connect to server (run) : telnet localhost [port]
                            --> the port should be consistent with the server [port]*/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <errno.h>
...
void chat_room(char *name, int socket_fd)
{
    ...
    while (1)
    {
        ...
        FD_SET(0, &clientFds);
        if (select(FD_SETSIZE, &clientFds, NULL, NULL, NULL) != -1) // use select function to check whether "fd" is available
        {
...
// format every message before sending it (this function will be used in both client and server)
...
void socket_connection(struct sockaddr_in *serverAddr, struct hostent *host, int socket_fd, long port)
{
